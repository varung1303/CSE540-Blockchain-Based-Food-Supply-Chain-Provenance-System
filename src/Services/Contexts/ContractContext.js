...
import RetailerContract from '../../Smart-Contract/ABI/Retailer.json';
  ...
  useEffect(() => {
    (async () => {
      if(authState.isWeb3Enabled){
        ...
        console.log("Available networks:", Object.keys(MainContract.networks));
        setNetworkId(networkId);

        if (!MainContract.networks[networkId]) {
          console.error(`Network ${networkId} not found in contract artifacts. Please deploy contracts to this network.`);
          alert(`Please connect MetaMask to network ID 5777 or 1337. Current network: ${networkId}`);
          return;
        }
        ...
        const retailer = new web3.eth.Contract(RetailerContract.abi, RetailerContract.networks[networkId].address);
        contractDispatch(contractStateRetailer(retailer));
        const stats = {};
    ...
    (async () => {
      if(authState.isAuthenticated && contractState.mainContract){
        ...
        if( role === "farmer"){
          ...
          const retailer = new web3.eth.Contract(RetailerContract.abi, RetailerContract.networks[networkId].address);
          contractDispatch(contractStateStakeholder(retailer));
        }
        else {
