...
contract Product {
  ...
  function add(
    ...
    require (_expiryDate > _manufacturingDate, "Product::add: Expiry date must be after manufacturing date");
    _items[_id] = Item({
      id: _id,
      title: _title,
  ...
  function isExpired(uint256 _id) public view returns (bool){
    return block.timestamp > _items[_id].expiryDate;
  }

  function getDaysUntilExpiry(uint256 _id) public view returns (uint256){
    if (block.timestamp >= _items[_id].expiryDate) {
      return 0;
    }
    return (_items[_id].expiryDate - block.timestamp) / 86400; // Convert seconds to days
  }
